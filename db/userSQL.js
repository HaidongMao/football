...
var UserSQL = {  
	...
	queryTime:'select train_date,sec div 60 as mins,sec mod 60 as secs from (select date_format(create_time,"%Y-%m-%d") train_date,sum(TIME_TO_SEC(end_time)-TIME_TO_SEC(begin_time)) as sec from football.train_log  where create_time>current_date() group by date_format(create_time,"%Y-%m-%d")) a;',
